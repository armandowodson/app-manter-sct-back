...
public class PontoTaxiController {
    ...
    @GetMapping("/imprimir")
    public ResponseEntity<byte[]> imprimirPontosEstacionamentosMotoTaxi(
                                                     @RequestParam(required = false) String numeroPonto,
                                                     @RequestParam(required = false) String descricaoPonto,
                                                     @RequestParam(required = false) String fatorRotatividade,
                                                     @RequestParam(required = false) String numeroVagas,
                                                     @RequestParam(required = false) String referenciaPonto,
                                                     ...
                                                     @RequestParam(required = true) Integer pageSize) {
        PageRequest pageRequest = PageRequest.of(pageIndex, pageSize);
        byte[] fileBytes = service.imprimirPontosEstacionamentosMotoTaxi(numeroPonto, descricaoPonto, fatorRotatividade,
                numeroVagas, referenciaPonto, pageRequest);

        Random gerador = new Random();
        Integer numAleatorio = gerador.nextInt(100);
        String fileName = "pontosEstacionamentos-" + LocalDate.now() + ":" + numAleatorio + ".pdf";
